				...
				<div class="c_content" :key='list.id' v-for='(list,listIndex) in item.spArr'>
					<div class="c_c_list">
						...
						<div class="list_right">
							<div class="right_top">
								<div style="width: 3rem;flex-grow: 1;"><span>{{list.productName}}&nbsp;[{{list.skuName}}]</span></div>
					...
					this.postRequest('customer/QueryCustomerRolelist', {
						...
						if(resp.data.state == 0 && resp.data.aaData.length > 0) {
							...
							if(this.$root.userInfo.extend=='004'){
								if(resp.data.aaData[0].extend == '005'||resp.data.aaData[0].extend == '0'){
									flg=true
								}else{
									this.$dialog.alert({ mes: '该手机号码非个人用户，请重新输入~~' })
								}
							}else{
								if(resp.data.aaData[0].extend == '002'||resp.data.aaData[0].extend == '004'||resp.data.aaData[0].extend == '0'){
									flg=true
								}else{
									this.$dialog.alert({ mes: '该手机号码非二批或门店用户，请重新输入~~' })
								}
							}
							if(flg) {
								this.$dialog.confirm({
							...
							this.$dialog.confirm({
								...
								opts: () => {
									...
									//customerRegist
									this.postRequest('customer/customerQuickRegist', {
										name: this.phoneVal,
...
//													})
			...
			submitOrder() {
				...
				if(flg) {
					...
					if(this.$root.userInfo.typeId == 'buyer') {
						...
						console.log(this.buyerInfo.customerId)
			...
			creatOrder() {
				if(this.creatOrderNum >= this.quickSettlementParam.length) {
					...
					//创建订单
					this.postRequest('order/createOrder', param).then(resp => {
						if(resp.data && resp.data.state == 0) {
							var arr = [];
			...
			filterSpArr(arr) {
				...
				for(let a = 0; a < arr.length; a++) {
					var o = {
						...
						"oriPrice": this.$root.userInfo.typeId == 'merchant' ? (parseFloat(arr[a].promotionPrice) * 100 / arr[a].skuWeight).toFixed(2) : (parseFloat(arr[a].promotionBasePrice) * 100).toFixed(2),
						"price": (parseFloat(arr[a].promotionPrice) * 100).toFixed(2),
					};
					newArr.push(o)
				}
				return newArr
