				...
				<div v-if='obj.quarantPicUrl' class="content_list" >
					...
					<div class="list_right">
						...
						<div class="list_children">
							...
							<span>
								<img @click="lookImg($root.config.imgURL+'/'+obj.quarantPicUrl)" class="list_pic_img" :src="$root.config.imgURL+'/'+obj.quarantPicUrl" alt="" />
							</span>
						</div>
					</div>
				</div>
		...
		methods: {
			...
			init(){
				this.postRequest('order/querySuYuanInfoByCode',{
					...
					if(resp.data.aaData&&resp.data.aaData!=null){
//						this.obj=d.aaData
						this.obj=Object.assign({},this.obj,resp.data.aaData)
...
<style lang='scss'>
	#qrZs {
		.header {
			...
			background: url(../../assets/firstImgs/banner.png) no-repeat;
